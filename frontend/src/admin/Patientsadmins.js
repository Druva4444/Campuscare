...
const handleUpload = async (event) => {
  ...
  try {
    const response = await axios.post('http://localhost:3020/studentupload', formData);
      ...
      async function fetchData() {
        if (gmail && college) { // Ensure gmail and college are set before making the API call
          try {
            ...
            setcompleted(response.data.accapp)
          } catch (error) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    navigate('/addstudent',{state:{college}});
  };
  ...
  return (
    <div className="nonnavbar340" style={{ overflowY: 'auto' }}>
      ...
      {students.map((student,index)=><Mainbody key={index} appointments={completed[index]} std ={student} upcoming={upcoming[index]} role='patient'/>)}
